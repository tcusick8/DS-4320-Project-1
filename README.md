...
> [!TIP]
> Background reading materials are available in the [Background Readings](<Background Readings/>)
...
erDiagram
  INCIDENTS {
    BIGINT CAD_INCIDENT_ID PK
    VARCHAR INCIDENT_DATETIME
    VARCHAR HELD_INDICATOR
    INTEGER INCIDENT_DISPOSITION_CODE
  }
  SEVERITY {
    BIGINT CAD_INCIDENT_ID FK
    VARCHAR INITIAL_CALL_TYPE
    FLOAT INITIAL_SEVERITY_LEVEL_CODE
    VARCHAR FINAL_CALL_TYPE
    FLOAT FINAL_SEVERITY_LEVEL_CODE
  }
  DISPATCH {
    BIGINT CAD_INCIDENT_ID FK
    VARCHAR VALID_DISPATCH_RSPNS_TIME_INDC
    INTEGER DISPATCH_RESPONSE_SECONDS_QY
    INTEGER INCIDENT_RESPONSE_SECONDS_QY
    BINARY FIRST_TO_HOSP_DATETIME
  }
  LOCATION {
    BIGINT CAD_INCIDENT_ID FK
    VARCHAR BOROUGH
  ...
  INCIDENTS ||--|| SEVERITY  : "CAD_INCIDENT_ID"
  INCIDENTS ||--|| DISPATCH  : "CAD_INCIDENT_ID"
  INCIDENTS ||--|| LOCATION  : "CAD_INCIDENT_ID"
```
